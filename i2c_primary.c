...
void i2c_init(void) {
	...
	DDRB |= (1 << I2C_SDA_PIN) | (1 << I2C_SCL_PIN);

	// Set both lines high (idle)
	PORTB |= (1 << I2C_SDA_PIN) | (1 << I2C_SCL_PIN);
...
void i2c_start(void) {
	...
	PORTB |= (1 << I2C_SDA_PIN); // SDA high
	PORTB |= (1 << I2C_SCL_PIN); // SCL high
	while (!(PORTB & (1 << I2C_SCL_PIN)));

	PORTB &= ~(1 << I2C_SDA_PIN); // SDA low
	_delay_us(I2C_DELAY_LONG);

	PORTB &= ~(1 << I2C_SCL_PIN); // SCL low
	PORTB |= (1 << I2C_SDA_PIN);  // SDA high (ready for data)
}
...
uint8_t i2c_transfer(uint8_t usisr_mask) {
	PORTB &= ~(1 << I2C_SCL_PIN); // Ensure SCL low
	USISR = usisr_mask;
...
uint8_t i2c_write_byte(uint8_t data) {
	...
	DDRB &= ~(1 << I2C_SDA_PIN); // SDA input for ACK
	uint8_t ack = i2c_transfer(USISR_1BIT_TRANSFER);
	DDRB |= (1 << I2C_SDA_PIN);  // SDA output again

	return ack;
}

uint8_t i2c_read_byte(uint8_t ack) {
	DDRB &= ~(1 << I2C_SDA_PIN); // Set SDA as input
	uint8_t data = i2c_transfer(USISR_8BIT_TRANSFER);
	DDRB |= (1 << I2C_SDA_PIN);  // Set SDA as output
